...
func NewNavigationPanel() *NavigationPanel {
	return &NavigationPanel{items: []string{"▾ Terraform", "  ▾ AWS", "    ◈ autoscaling-group  🔴", "    ◈ aws-backup  🟢", "    ◈ certificate-manager  🟢", "    ◈ cloudfront  🟡", "    ◈ kms-key  🟢", "  ▸ Azure", "  ▸ GCP", "  ▸ OpenTofu", "  ▸ Helm", "", "◉ Environments", "◉ Workspaces", "◉ History", "◉ Projects", "◉ Search"}}
}
func (p *NavigationPanel) Init() tea.Cmd     { return nil }
func (p *NavigationPanel) SetSize(w, h int)  { p.width, p.height = w, h }
...
func (p *NavigationPanel) View() string {
	rows := []string{lipgloss.NewStyle().Bold(true).Foreground(styles.ColorAccent).Render("NAVIGATION"), ""}
	for i, it := range p.items {
		st := lipgloss.NewStyle()
		if i == p.cursor {
			st = st.Background(styles.ColorAccent).Foreground(lipgloss.Color("#FFFFFF"))
		}
		rows = append(rows, st.Render(it))
	}
