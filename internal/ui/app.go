...
func NewApp() *App {
	return &App{focused: FocusNav, nav: panels.NewNavigationPanel(), main: panels.NewMainPanel(), ctx: panels.NewContextPanel(), logs: panels.NewLogsPanel()}
}
...
func (a *App) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "q", "ctrl+c":
			return a, tea.Quit
		case "tab":
			a.focused = (a.focused + 1) % 4
			a.setFocus()
		case "shift+tab":
			a.focused = (a.focused + 3) % 4
			a.setFocus()
		}
	}
	for _, p := range []PanelComponent{a.nav, a.main, a.ctx, a.logs} {
		_, _ = p.Update(msg)
	}
	return a, nil
}
func (a *App) View() string {
	...
	a.nav.SetSize(leftW, topH)
	a.main.SetSize(centerW, topH)
	a.ctx.SetSize(rightW, topH)
	a.logs.SetSize(a.width, bottomH)
...
func (a *App) setFocus() {
	a.nav.SetFocused(a.focused == FocusNav)
	a.main.SetFocused(a.focused == FocusMain)
	a.ctx.SetFocused(a.focused == FocusContext)
	a.logs.SetFocused(a.focused == FocusLogs)
}
